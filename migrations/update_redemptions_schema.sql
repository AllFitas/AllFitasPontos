-- 1. ADICIONA A COLUNA DE QUANTIDADE NA TABELA DE RESGATES
ALTER TABLE redemptions ADD COLUMN IF NOT EXISTS quantity INTEGER DEFAULT 1;

-- 2. ATUALIZA A FUNÇÃO DE RESGATE (FIFO) PARA SUPORTAR QUANTIDADE FÍSICA
...
BEGIN
  ...
  -- Verifica se conseguiu resgatar tudo (opcional, para evitar saldo negativo "fantasma")
  IF v_remaining_to_redeem > 0 THEN
    RAISE EXCEPTION 'Saldo insuficiente nos últimos 30 dias para completar o resgate.';
  END IF;

  -- 3. REGISTRA A SAÍDA NO HISTÓRICO (AGORA COM QUANTIDADE FÍSICA)
  INSERT INTO redemptions (customer_name, product_id, points_spent, quantity, created_at)
  VALUES (p_customer_name, p_product_id, p_points_to_redeem, p_quantity, NOW());
